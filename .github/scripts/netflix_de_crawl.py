...
def post(variables, attempts=8):
  ...
  for n in range(1,attempts+1):
    req=request.Request(ENDPOINT,data=body,headers={'content-type':'application/json','accept':'application/json','user-agent':'what2watch-netflix-de/partition-1.1','origin':'https://www.justwatch.com','referer':'https://www.justwatch.com/'},method='POST')
    try:
...
def save(out,target,rows,seen,errors,stats,genres,oversized,final=False):
  ...
  return report
def crawl_partition(name,filt,rows,seen,errors,stats,out,target,genres,oversized):
  ...
  if total>HARD_WINDOW: print(f'SKIP_OVERSIZE {name} total={total}',flush=True); return total,False
  offset=0; local=set()
  while offset<total:
    conn=first if offset==0 else fetch_page(filt,offset)
    if conn.get('totalCount')!=total: errors.append({'partition':name,'offset':offset,'error':f'total changed {total}->{conn.get("totalCount")}' }); return total,False
    edges=conn.get('edges') or []
    if not edges: errors.append({'partition':name,'offset':offset,'error':'empty page'}); return total,False
    for e in edges:
      r=norm(e.get('node') or {}); key=r.get('justwatch_id')
      if not key: continue
      local.add(key)
      if key not in seen: seen.add(key); rows[key]=r
    offset+=len(edges); time.sleep(.45)
  ok=len(local)==total
  if not ok: errors.append({'partition':name,'error':f'local unique {len(local)} != total {total}'})
  save(out,target,rows,seen,errors,stats,genres,oversized,False)
  print(f'DONE {name} local={len(local)} global={len(seen)}',flush=True); return total,ok
def main():
  out=Path('out'); out.mkdir(exist_ok=True); rows={}; seen=set(); errors=[]; stats=[]; oversized=[]
  baseline=fetch_page(base_filter(),0); target=baseline.get('totalCount'); genres=set(); offset=0
  ...
  while offset<HARD_WINDOW and offset<target:
    ...
    offset+=len(edges); time.sleep(.40)
  print('GENRES '+','.join(sorted(genres)),flush=True)
  for typ in ('MOVIE','SHOW'):
    for g in sorted(genres):
      total,ok=crawl_partition(f'{typ}:genre:{g}',base_filter({'objectTypes':[typ],'genres':[g]}),rows,seen,errors,stats,out,target,genres,oversized)
      if total>HARD_WINDOW: oversized.append((typ,g,total))
  # The only oversized partitions observed are MOVIE comedy/drama. Any title in them that has another genre is already covered above.
  # Isolate the remaining comedy/drama-only core by excluding every other discovered genre.
  other=sorted(genres-{'cmy','drm'})
  for g in ('cmy','drm'):
    crawl_partition(f'MOVIE:core:{g}',base_filter({'objectTypes':['MOVIE'],'genres':[g],'excludeGenres':other}),rows,seen,errors,stats,out,target,genres,oversized)
  # Titles with no discovered genre at all (or only an unseen genre) are isolated by excluding the complete discovered vocabulary.
  for typ in ('MOVIE','SHOW'):
    crawl_partition(f'{typ}:no-known-genre',base_filter({'objectTypes':[typ],'excludeGenres':sorted(genres)}),rows,seen,errors,stats,out,target,genres,oversized)
  report=save(out,target,rows,seen,errors,stats,genres,oversized,True)
  print(json.dumps({k:v for k,v in report.items() if k!='partition_stats'},ensure_ascii=False,indent=2),flush=True)
  if not report['coverage_complete']: print('COVERAGE_COMPLETE=false',file=sys.stderr); sys.exit(2)
