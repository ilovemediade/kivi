...
def post(variables, attempts=8, query=QUERY, operation='ProviderCatalog'):
    ...
    for n in range(1,attempts+1):
        req=request.Request(ENDPOINT,data=body,headers={
            ...
            'user-agent':'what2watch-disney-de/partition-1.0',
...
def detect_disney_package():
    ...
    for p in packages:
        ...
        if name in ('disney plus','disney+') or tech in ('disneyplus','disney plus') or short=='dnp':
            cand.append(p)
    print('DISNEY_PACKAGE_CANDIDATES '+json.dumps(cand,ensure_ascii=False),flush=True)
    exact=[p for p in cand if str(p.get('clearName') or '').strip().lower() in ('disney plus','disney+')]
    chosen=(exact or cand)
    if len(chosen)!=1:
        raise RuntimeError(f'Could not uniquely resolve Disney+ DE package: {chosen}')
    p=chosen[0]
    code=str(p.get('shortName') or '').strip()
    if not code: raise RuntimeError(f'Disney package has no shortName: {p}')
    return code,p

def base_filter(extra=None):
    if not PACKAGE: raise RuntimeError('Disney package not initialized')
...
def save(out,target,rows,seen,errors,stats,genres,final=False):
    ...
    coverage=final and target>3000 and len(seen)==target and len(records)==target and len(verified)==target and not rejected and not errors
    report={
        'provider':'Disney+','region':'DE','scope':f'Disney+ subscription catalog / JustWatch {PACKAGE} FLATRATE only',
    ...
    (out/'disney-de-catalog.json').write_text(json.dumps({'report':report,'records':records},ensure_ascii=False,indent=2),encoding='utf-8')
    (out/'disney-de-crawl-report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    return report
...
def crawl_partition(name,filt,rows,seen,errors,stats,out,target,genres):
    ...
    while offset<total:
        ...
        offset+=len(edges); time.sleep(.30)
    ok=len(local)==total
...
def main():
    ...
    out=Path('out-disney'); out.mkdir(exist_ok=True); rows={}; seen=set(); errors=[]; stats=[]; genres=set()
    PACKAGE,PACKAGE_META=detect_disney_package()
    print(f'DISNEY_PACKAGE {PACKAGE} {PACKAGE_META.get("clearName")}',flush=True)
    baseline=fetch_page(base_filter(),0); target=baseline.get('totalCount')
    if not target or target<=3000: raise SystemExit(f'implausible Disney+ DE target total: {target} package={PACKAGE_META}')
    ...
    while offset<HARD_WINDOW and offset<target:
        ...
        offset+=len(edges); time.sleep(.25)
